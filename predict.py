...
def spatial_label_propagation(edges, user_locations, user_predictions):
    """
    ...
    if user_predictions:
        user_predictions = user_predictions.mapValues(lambda x: (x['latitude'], x['longitude']))
        all_locations = user_locations.union(user_predictions.subtractByKey(user_locations))
    else:
        all_locations = user_locations
    
    neighbor_locations = edges.join(all_locations).values().groupByKey()
...
if __name__ == "__main__":
    ...
    if config['params']['coalesce']:
        user_locations = user_locations.coalesce(config['params']['coalesce'])
    
    user_locations = user_locations.map(parse_user_location)
    ...
    edges = sc.textFile(config['graph'])
    if config['params']['coalesce']:
        edges = edges.coalesce(config['params']['coalesce'])
    
    edges = edges.map(parse_wedge)
    ...
    for _ in range(config['params']['num_iter']):
        user_predictions = spatial_label_propagation(edges, user_locations, user_predictions)
        if config['params']['coalesce']:
